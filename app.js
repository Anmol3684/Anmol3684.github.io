...
function btnclicked()
{
const searchText = document.querySelector("#txt-search").value
   console.log(`The name is: ${searchText}`)
   const searchResult = []; 
 for (const video of videosList) {
  if (video.description.includes(searchText))
    {
    searchResult.push(video)   
    generateRowHTML(searchResult)
...
function btnshowall()
{
  ...
  {
    searchResult.push(videosList[i]) 
  generateRowHTML(videosList[i])
  }
  const combined = searchResult.map((result) => generateRowHTML(result)).join('\n')
  console.log(list)
  list.innerHTML = combined
...
document.querySelector("#btn-search").addEventListener("click", btnclicked)
document.querySelector("#btn-show-all").addEventListener("click", btnshowall)